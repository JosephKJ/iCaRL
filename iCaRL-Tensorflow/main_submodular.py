...
for itera in range(nb_groups):
  ...
  files_from_cl = files_train[itera]
  inits, scores, label_batch, loss_class, file_string_batch, op_feature_map, softmax_out= \
      utils_icarl.reading_data_and_preparing_network_with_softmax(files_from_cl, gpu, itera, batch_size, train_path,
                                                     labels_dic, mixing, nb_groups, nb_cl, save_path)
  ...
  with tf.Session(config=config) as sess:
    ...
    Dtot, processed_files, label_dico, softmax_prob = utils_icarl.load_class_in_feature_space_with_softmax(files_from_cl, batch_size, scores, label_batch, loss_class, file_string_batch, op_feature_map, sess, softmax_out)
    processed_files = np.array([x.decode() for x in processed_files])
    
    # Submobular Exemplar seletion procedure : ranking of the potential exemplars
    print('Exemplars selection starting ...')
    for iter_dico in range(nb_cl):
        ind_cl     = np.where(label_dico == order[iter_dico + itera*nb_cl])[0]
        set = processed_files[ind_cl]
        subset_size = nb_protos_cl
        penumtimate_acts = Dtot[:,ind_cl].T
        logits = softmax_prob[ind_cl]

        sampler = SubmodularSampler(set, subset_size, penumtimate_acts, logits)
        subset = sampler.get_subset()
        files_protoset[itera*nb_cl + iter_dico].extend(subset)
