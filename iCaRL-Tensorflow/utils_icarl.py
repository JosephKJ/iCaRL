...
def reading_data_and_preparing_network_with_softmax(files_from_cl, gpu, itera, batch_size, train_path, labels_dic, mixing, nb_groups,
                                       nb_cl, save_path):
    ...
    with tf.variable_scope('ResNet18'):
        with tf.device('/gpu:' + gpu):
            scores = utils_resnet.ResNet18(image_batch - mean_img, phase='test', num_outputs=nb_cl * nb_groups)
            softmax_out = tf.nn.softmax(scores[:,:(itera+1)*nb_cl])
            graph = tf.get_default_graph()
            op_feature_map = graph.get_operation_by_name('ResNet18/pool_last/avg').outputs[0]
    ...
    return inits, scores, label_batch, loss_class, file_string_batch, op_feature_map, softmax_out
...
def load_class_in_feature_space_with_softmax(files_from_cl, batch_size, scores, label_batch, loss_class, file_string_batch,
                                op_feature_map, sess, softmax_out):
    processed_files = []
    label_dico = []
    Dtot = []
    softmax_prob = []
    ...
    for i in range(int(np.ceil(len(files_from_cl) / batch_size) + 1)):
        sc, l, loss, files_tmp, feat_map_tmp, softmax_output = sess.run(
            [scores, label_batch, loss_class, file_string_batch, op_feature_map, softmax_out])
        processed_files.extend(files_tmp)
        label_dico.extend(l)
        softmax_prob.extend(softmax_output)
        mapped_prototypes = feat_map_tmp[:, 0, 0, :]
        Dtot.append((mapped_prototypes.T) / np.linalg.norm(mapped_prototypes.T, axis=0))
    ...
    return Dtot, processed_files, label_dico, softmax_prob
